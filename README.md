...
DeepSeek Harness 界面简化插件：进入清理模式后，右键移除页面元素，被移除的元素可在「设置 → 简化」中原样恢复。

Clean up the DeepSeek Harness (dsh) web UI: in clean mode, right-click any page element to remove it; removed elements persist and can be restored from the settings page.

## 功能 / Features

- 首页左下角、设置按钮上方的「简化」图标按钮，一键进入/退出清理模式（仅图标，无文字）。
- 清理模式中悬浮元素会显示红色描边，右键移除，左键操作完全不受影响，Esc 或再次点击按钮退出。
- 移除记录保存在浏览器 localStorage，并实时防复活（MutationObserver 重扫），刷新页面、React 重渲染都不会让被移除的元素回来。
- 设置页侧栏新增「简化」页：列出所有被移除元素，每条可独立「恢复」或勾选后「批量恢复」，恢复的元素以原样回到原位置。
...
4. 打开「设置 → 简化」，可查看全部已移除元素：勾选多条后点「批量恢复」，或对单条点「恢复」。
...
- `fixture-test.html` 在本地存在：用系统 Chrome 打开可对引擎逻辑做全量自检（移除/记录/恢复/重扫/清理模式/设置页渲染，38 项断言）。
